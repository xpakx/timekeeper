...
def create_user_hero(hero_id: int, user_id: int, skillset: bool = True, fainted: bool = False, damage: int = 0, poisoned: bool = False):
    db = TestingSessionLocal()
    item = UserHero(
            hero_id=hero_id,
            owner_id=user_id,
...
def create_poison_powder() -> int:
    db = TestingSessionLocal()
    skill = Skill(
            accuracy=75,
            power=0,
            priority=0,
            move_type=HeroType.poison,
            max_usages=35,
            name="Poison Powder",
...
def test_using_poison_powder(test_db):
    ...
    teach_skill(hero_id, create_poison_powder())
...
def test_miss(test_db):
    ...
    enemy: UserHero = db.query(UserHero).where(UserHero.id == enemy_id).first()
    db.close()
    assert not enemy.poisoned


def test_poison_damage(test_db):
    user_id = create_user_and_return_id()
    headers = {"Authorization": f"Bearer {get_token_for(user_id)}"}
    hero_id = create_user_hero(create_bulbasaur(), user_id)
    team_id = create_team(user_id)
    add_to_team(team_id, hero_id, 1)
    enemy_id = create_user_hero(create_charmander(), None, poisoned=True)
    battle_id = create_battle(user_id, hero_id, enemy_id)
    response = client.post(f"/battles/{battle_id}",
                           headers=headers,
                           json={
...
@patch('timekeeper.services.mechanics.battle_mech_service.test_accuracy', Mock(return_value=True))
def test_poison_type_invulnerability(test_db):
    user_id = create_user_and_return_id()
    headers = {"Authorization": f"Bearer {get_token_for(user_id)}"}
    hero_id = create_user_hero(create_bulbasaur(), user_id)
    team_id = create_team(user_id)
    add_to_team(team_id, hero_id, 1)
    teach_skill(hero_id, create_poison_powder())
    enemy_id = create_user_hero(create_charmander(hero_type=HeroType.poison), None)
    battle_id = create_battle(user_id, hero_id, enemy_id)
    response = client.post(f"/battles/{battle_id}",
                           headers=headers,
                           json={
...
@patch('timekeeper.services.mechanics.battle_mech_service.test_accuracy', Mock(return_value=True))
def test_steel_type_invulnerability(test_db):
    user_id = create_user_and_return_id()
    headers = {"Authorization": f"Bearer {get_token_for(user_id)}"}
    hero_id = create_user_hero(create_bulbasaur(), user_id)
    team_id = create_team(user_id)
    add_to_team(team_id, hero_id, 1)
    teach_skill(hero_id, create_poison_powder())
    enemy_id = create_user_hero(create_charmander(hero_type=HeroType.steel), None)
    battle_id = create_battle(user_id, hero_id, enemy_id)
    response = client.post(f"/battles/{battle_id}",
                           headers=headers,
                           json={

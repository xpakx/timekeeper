...
def create_growth() -> int:
    ...
    skill = Skill(
            ...
            secondary_stage_effect=StageEffect.special_attack,
            self_targetted=True
            )
    db.add(skill)
    db.commit()
...
def test_growth_damage(test_db):
    ...
    teach_skill(hero_id, create_growth())
...
def test_changing_stage(test_db):
    ...
    mods = battle.hero_mods
    db.close()
    assert mods.attack == 1
    assert mods.special_attack == 1
